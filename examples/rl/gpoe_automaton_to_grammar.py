...
def parse(
    ...
    # base = {
    #     str(p): (str(p.type).replace("(", "").replace(")", ""), 0)
    #     for p in dsl.list_primitives
    # }
    # base["ite"] = ("bool -> 'a[float|action] -> 'a -> 'a", 0)
    # for cst_type in constant_types:
    #     base[f"<{cst_type}>"] = (str(cst_type), 0)
    # gdsl = GDSL(base)
    # dfta = load_automaton_from_file(file)
    # str_type_req = str(target_type_request).replace("(", "").replace(")", "")
    # # add constants
    # for (P, args), dst in dfta.rules.copy().items():
    #     for cst_type in constant_types:
    ...
    #     for (P, args), dst in dfta.rules.copy().items():
    #         if P == "A0":
    #             for i in range(2, actions):
    #                 dfta.rules[(f"A{i}", args)] = dst
    # dfta.refresh_reversed_rules()
    # dfta: GDFTA[str, str] = specialize(dfta, str_type_req, gdsl)
    # dfta.reduce()
    # dfta = dfta.minimise()
