...
function gen_dataset(){
    ...
    if [ ! -f "$file" ]; then
        python examples/pbe/karel/karel_task_generator.py -o $file -s $1 -w 10 --height 10 -g 4 --size $TASKS --max-depth 10 --min-operations 9 --uniform --filter automatic_dfta_filter_karel.py
...
function run_exp(){
    ...
    if [ ! -f "$filename" ]; then
        ...
        python examples/pbe/solve.py --dsl karel -d karel/dataset.pickle --solver cutoff --search beap_search -t 60 -o karel/ $arg
        mv karel/dataset_beap_search_uniform_cutoff.csv $filename
    fi
}
